...
export const SaleCheckAll = forwardRef((props, ref) => {
    const {selled, returned, selledDiscounts, returnedDiscounts, selledPayments, returnedPayments, product} = props
    const {user, market} = useSelector(state => state.login)
    const {currencyType} = useSelector(state => state.currency)
    const calculateAllSum = (data) => {
        return data ? data.reduce((acc, pr) => {
            return acc + pr[currencyType === 'USD' ? 'totalprice' : 'totalpriceuzs']
        }, 0) : 0
    }
    const calculateAllDiscounts = (data) => {
        return data ? data.reduce((acc, pr) => {
            return acc + pr[currencyType === 'USD' ? 'discount' : 'discountuzs']
        }, 0) : 0
    }
    const calculateAllPayments = (data) => {
        return data ? data.reduce((acc, pr) => {
            return acc + pr[currencyType === 'USD' ? 'payment' : 'paymentuzs']
        }, 0) : 0
    }
    return (
        <div ref={ref} className={'bg-white-900 p-4 rounded-md'}>
            <div className='flex pb-2 justify-between border-b-[0.8px] border-black-700'>
                <ul className='w-[35%]'>
                    ...
                    <li className='check-ul-li'>Telefon:
                        <span className='check-ul-li-span'>{market.phone1}</span>
                    </li>
                ...
                <div className='check-ul-li flex-col'>
                    <div className={'grow text-center'}>
                        <h2 className='check-text-style mb-5'>Sotuv: <span
                            className={'ml-2'}>{product?.id}</span></h2>
                    </div>
                    <div className='check-ul-li justify-end'>
                        <p>Sotuvchi: <span className='check-ul-li-span'>{user.firstname} {user.lastname}</span></p>
                    </div>
                </div>
            </div>
            {selled?.length > 0 && <div className='mt-5'>
                <h3 className='text-[1.3rem] text-black-700 mb-5 font-bold'>Sotilganlar :</h3>
            ...
            <div className='border-t-[0.8px] border-black-700 w-full mt-4 mb-4 text-end'>
                <h3 className='text-[1.1rem] text-black-700 font-bold pt-4'>Sotilganlar jami
                    : <span>{calculateAllSum(selled).toLocaleString('ru-Ru')} {currencyType}</span></h3>
            </div>
            {returned?.length > 0 && <>
                <div className='mt-5'>
                    <h3 className='text-[1.3rem] text-black-700 mb-5 font-bold'>Qaytarilganlar :</h3>
                    <table className='border-collapse border border-slate-400 w-full'>
                        <thead>
                        <tr>
                            <td className='check-table-rtr'>№</td>
                ...
                <div className='border-t-[0.8px] border-black-700 w-full mt-4 mb-4 text-end'>
                    <h3 className='text-[1.1rem] text-black-700 font-bold pt-4'>Qaytarilganlar jami
                        : <span>{calculateAllSum(returned).toLocaleString('ru-Ru')} {currencyType}</span>
                    </h3>
                </div>
            </>}
            <div className='border-t-[0.8px] border-black-700 w-full my-[1.5rem]'></div>
            <ul>
                <li className='check-ul-li-foot border-t-0'> Jami: <span>{(calculateAllSum(selled) + calculateAllSum(returned)).toLocaleString('ru-Ru')} {currencyType}</span>
                </li>
                <li className='check-ul-li-foot'> Chegirma: <span>{(calculateAllDiscounts(selledDiscounts) + calculateAllDiscounts(returnedDiscounts)).toLocaleString('ru-Ru')} {currencyType}</span>
                </li>
                <li className='check-ul-li-foot'> To'langan: <span>{(calculateAllPayments(selledPayments) + calculateAllPayments(returnedPayments)).toLocaleString('ru-Ru')} {currencyType}</span>
                </li>
                <li className='check-ul-li-foot'> Qarz: <span>{((calculateAllSum(selled) + calculateAllSum(returned)) - (calculateAllPayments(selledPayments) + calculateAllPayments(returnedPayments)) - (calculateAllDiscounts(selledDiscounts) + calculateAllDiscounts(returnedDiscounts))).toLocaleString('ru-Ru')} {currencyType}</span>
                </li>
            </ul>
        </div>
    )
