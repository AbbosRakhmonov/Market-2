...
export const SaleCheck = forwardRef((props, ref) => {
    ...
    const calculateDebt = (total, payment, discount = 0) => {
        return total - payment - discount
    }
    return (<div ref={ref} className={'bg-white-900 p-4 rounded-md'}>
            ...
            <div className='check-ul-li flex-col'>
                <div className={'grow text-center'}>
                    <h2 className='check-text-style mb-5'>Sotuv: <span
                        className={'ml-2'}>{product.saleconnector.id}</span></h2>
                    <h2 className='check-text-style'>Chek: {product.id}</h2>
                </div>
                    ...
                    product?.products.map((item, index) => {
                        return (
                            <tr key={index}>
                                ...
                                <td className='check-table-body'>{currencyType === 'USD' ? item.totalprice : item.totalpriceuzs} {currencyType}</td>
                            </tr>
                        )
                    })
                }
        ...
        <ul>
            <li className='check-ul-li-foot border-t-0'> Jami: <span>{currencyType === 'USD' ? product.payment.totalprice : product.payment.totalpriceuzs} {currencyType}</span>
            </li>
            <li className='check-ul-li-foot'> Chegirma: <span>{product.hasOwnProperty('discount') ? currencyType === 'USD' ? product.discount.discount : product.discount.discountuzs : 0} {currencyType}</span>
            </li>
            <li className='check-ul-li-foot'> To'langan: <span>{currencyType === 'USD' ? product.payment.payment : product.payment.paymentuzs} {currencyType}</span>
            </li>
            <li className='check-ul-li-foot'> Qarz: <span>{currencyType === 'USD' ? calculateDebt(product.payment.totalprice, product.payment.payment, product?.discount?.discount) : calculateDebt(product.payment.totalpriceuzs, product.payment.paymentuzs, product?.discount?.discountuzs)} {currencyType}</span>
            </li>
        </ul>
    </div>)
})
