...
function AccordionLink({
    ...
    return (
        <>
            <button
                className={`navbar__link w-full flex items-center ${
                    expanded ? 'justify-center' : 'justify-between'
                } px-[0.9375rem] py-[0.625rem] rounded-[1.875rem] transition ease-in-out duration-200 ${
                    path.includes(globalPath[0])
                ...
                {!expanded && (
                    <IoChevronDown
                        className={`transition ease-in-out duration-200 ${
                            activeFirstSubMenuId === id ? 'rotate-180' : ''
                        }`}
                        size={'1.5rem'}
                    />
                )}
            ...
            <div
                className={`transition-all ease-in-out duration-300 pl-[1.875rem] flex flex-col gap-[0.625rem] overflow-hidden ${
                    activeFirstSubMenuId === id
