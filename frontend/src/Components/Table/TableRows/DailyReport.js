    ...
    return (
        <>
            {map(data, (dailyReport, index) => {
                const {
                    product: {productdata: {name, code}, unit},
                    pieces,
                    createdAt,
                    unitprice,
                    unitpriceuzs,
                ...
                return (
                    <tr className='tr' key={uniqueId('daily-report-row')}>
                        <td className='text-left td py-[0.375rem]'>{index + 1}</td>
                        <td className='td'>
                            <span className={'flex justify-between w-full'}>
                                <p> {new Date(createdAt).toLocaleDateString('ru-RU')} </p>
                                <p>{new Date(createdAt).toLocaleTimeString('ru-RU', {
                                    hour: '2-digit',
                                    minute: '2-digit',
                                    hourCycle: 'h24'
                                })}</p>
                        ...
                        <td className='text-left td'>{saleconnector?.client?.name}</td>
