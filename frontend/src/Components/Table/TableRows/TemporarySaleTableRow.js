    ...
    <>
      {data.map((sale, index) => (
        <tr className='tr' key={uniqueId('sale')}>
          ...
          <td className='td text-left'>{sale.client.name || sale.client.id}</td>
          <td className='td text-right'>
            {sale.sales.reduce((prev, product) => prev + product.pieces, 0)}
          </td>
          <td className='text-success-500 td text-right'>
            {sale.sales.reduce((prev, product) => prev + product.totalprice, 0)}{' '}
            {currency}
          </td>
          <td className='td text-right'>
