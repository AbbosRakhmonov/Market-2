    ...
    <>
      {data.map((incoming, index) => (
        <tr className='tr' key={incoming._id}>
          ...
          <td className='td text-right'>{incoming.productdata.code}</td>
          <td className='td text-left'>{incoming.productdata.name}</td>
          <td className='td text-right'>
            {(editedIncoming._id === incoming._id && (
              ...
              <span>
                {incoming.total}{' '}
                <span className='text-warning-500'>{incoming.unit.name}</span>
              </span>
            )}
          </td>
          <td className='td text-right'>
              ...
              {editedIncoming._id === incoming._id ? (
                <>
                  <TableBtn
                    ...
                    onClick={() => saveEditIncoming(incoming)}
                  />
                </>
              ) : (
                <>
