...
import {getFilialIdProducts, getExchangesFilter, clearSearchedExchanges} from './ProductIdExchangesSlice'
...
function ProductIdExchanges() { 
    ...
    const {idProducts, loading, total, totalSearched,searchedExchanges} = useSelector((state) => state.exchangesIdData)
    const headers = [
        { styles: 'w-[10%] text-start', title: '№' },
        { styles: 'w-[10%] text-start', filter: 'category.code', title: 'Kategoriyasi' },
        { styles: 'w-[10%] text-start', filter: 'productdata.code', title: 'Kodi' },
    ...
    const filterByName = (e) => {
        ...
        if (valForSearch === '') {
            ...
            const filteredExchanges = filter(idProducts, (item) => {
                return item?.productdata?.name.toLowerCase().includes(valForSearch)
            })
            setData(filteredExchanges)
            setFilteredDataTotal(filteredExchanges.length)
        }
        ...
        if (e.key === 'Enter') {
            ...
            const body = {
                ...
                search: {
                    ...
                    categoryofproduct: searchByCategory.replace(/\s+/g, ' ').trim(),
                }
            }
    ...
    const exportData = () => {
        ...
        const exportHeader = ['№', 'Kategoriyasi','Kodi', 'Maxsulot Nomi', 'Maxsulot Soni', 'Kelgan Narxi UZS', 'Jami Summasi UZS']
