...
module.exports.getSalesDataByMonth = async (req, res) => {
  try {
    ...
    saleconnectors.map((sale) => {
      const totalprice = sale.payments.reduce((prev, { totalprice }) => {
        return prev + (totalprice || 0);
      }, 0);
      const totalpriceuzs = sale.payments.reduce((prev, { totalpriceuzs }) => {
      ...
      const incomingtotaluzs = sale.products.reduce((prev, product) => {
        return prev + product.price.incomingpriceuzs * product.pieces;
      }, 0);
      ...
      const discountuzs = sale.discounts.reduce((prev, discount) => {
        return prev + discount.discountuzs;
      }, 0);

      monthProfit.usd += totalprice - incomingtotal - discount;
      monthProfit.uzs += totalpriceuzs - incomingtotaluzs - discountuzs;
    });
